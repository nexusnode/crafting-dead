...
public class GunAnimationReloadFiveSeven extends GunAnimationReload {
  ...
  public GunAnimationReloadFiveSeven() {
    this.ejectingClip = true;
    trans1 = 0F;
  }

  public GunAnimationReloadFiveSeven(boolean par1) {
  ...
  public void doRender(ItemStack par1, float par2, MatrixStack matrixStack) {
    ...
    matrixStack.mulPose(new Vector3f(-0.5F, 0.5F, -0.2F).rotationDegrees(-progress));
  ...
  public void doRenderHand(ItemStack par1, float par2, boolean par3, MatrixStack matrixStack) {
    if (par3) {
      ...
      float transprogress = lastTrans1 + (trans1 - lastTrans1) * par2;
      matrixStack.translate(-transprogress * -0.8F, transprogress * 0.5F, transprogress);
    ...
    if (!this.ejectingClip && !par3 && ticks >= (getMaxAnimationTick() / 2)) {
      ...
      float y = this.armMovement - 0.08F;
