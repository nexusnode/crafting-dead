...
public class GunAnimationReloadMossberg extends GunAnimationReload {

  protected float rotation1 = 0F;
  protected float lastRotation1 = 0F;
  protected float maxRotation1 = 50;
  ...
  public GunAnimationReloadMossberg() {
    this.ejectingClip = true;
    trans1 = 0F;
  }

  public GunAnimationReloadMossberg(boolean par1) {
  ...
  public void doRender(ItemStack par1, float par2, MatrixStack matrixStack) {
    ...
    matrixStack.mulPose(new Vector3f(1.0F, 0.0F, 0.15F).rotationDegrees(-progress));
  }

  @Override
  public void doRenderHand(ItemStack par1, float par2, boolean par3, MatrixStack matrixStack) {

    if (!this.ejectingClip && !par3 && this.ticks >= (getMaxAnimationTick() / 2)) {

      float ticks = this.ticks - (getMaxAnimationTick() / 2);

      float x = this.armMovement - 0.35F;
      float y = this.armMovement - 0.30F;
