...
public class BackpackContainer extends Container {
    ...
    public ItemStack slotClick(int slotId, int dragType, ClickType clickTypeIn, PlayerEntity player) {
        Slot tmpSlot = slotId >= 0 && slotId < this.inventorySlots.size() ? this.inventorySlots.get(slotId) : null;
        if (tmpSlot != null && tmpSlot.inventory == player.inventory && tmpSlot.getSlotIndex() == player.inventory.currentItem) {
            return tmpSlot.getStack();
        }
        if (clickTypeIn != ClickType.SWAP) return super.slotClick(slotId, dragType, clickTypeIn, player);
        ItemStack stack = player.inventory.getStackInSlot(dragType);
        if (stack != player.inventory.getItemStack()) return super.slotClick(slotId, dragType, clickTypeIn, player);
