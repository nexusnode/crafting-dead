package com.craftingdead.mod.world.gen.layer;

import java.util.ArrayList;
import java.util.EnumMap;
import java.util.List;

import com.craftingdead.mod.init.ModBiomes;
...
public class GenLayerZombie extends GenLayer {
	...
	public GenLayerZombie(long worldSeed, GenLayer parentLayer) {
		...
		biomeMap.get(BiomeType.DESERT).add(new BiomeEntry(ModBiomes.ZOMBIE_DESERT, 10));
		biomeMap.get(BiomeType.WARM).add(new BiomeEntry(ModBiomes.ZOMBIE_PLAINS, 10));
		biomeMap.get(BiomeType.COOL).addAll(ImmutableList.of(new BiomeEntry(ModBiomes.ZOMBIE_FOREST, 10),
				new BiomeEntry(ModBiomes.ZOMBIE_WASTELAND, 10)));
		biomeMap.get(BiomeType.ICY).add(new BiomeEntry(ModBiomes.ZOMBIE_SNOW, 10));
