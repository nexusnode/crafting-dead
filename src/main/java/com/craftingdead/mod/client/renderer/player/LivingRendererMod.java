package com.craftingdead.mod.client.renderer.player;

import com.craftingdead.mod.client.renderer.entity.model.BipedModelMod;
import com.craftingdead.mod.client.renderer.entity.model.PlayerModelMod;
  ...
  protected void applyRotations(AbstractClientPlayerEntity entityLiving, float ageInTicks,
    ...
    if (entityLiving.isElytraFlying()) {
      ...
      if (d0 > 0.0D && d1 > 0.0D) {
        ...
        GlStateManager
            .rotatef((float) (Math.signum(d3) * Math.acos(d2)) * 180.0F / (float) Math.PI, 0.0F,
                1.0F, 0.0F);
      }
    } else if (f > 0.0F) {
      super.applyRotations(entityLiving, ageInTicks, rotationYaw, partialTicks);
